...
def parallel_plot(x1:np.ndarray,y1:np.ndarray,x2:np.ndarray,y2:np.ndarray,
    ...
    if (x1.shape != y1.shape) or  (min(x1.shape)==0) or (len(x1)==1):
        return None
    if (x2.shape != y2.shape) or  (min(x2.shape)==0) or (len(x1)==1):
        return None

    if(all(i < j for i, j in zip(x2, x2[1:]))):
        pass
    elif(all(i > j for i, j in zip(x2, x2[1:]))):
        pass
    else:
        return None

    if(all(i < j for i, j in zip(x1, x1[1:]))):
        pass
    elif(all(i > j for i, j in zip(x1, x1[1:]))):
        pass
    else:
        return None
